...
bars = plt.barh(y_pos, df2_sorted['MUSIQ_Mean'], 
                xerr=df2_sorted['MUSIQ_Std'], 
                capsize=3, alpha=0.7,
                color=colors,
                edgecolor=['darkred' if c=='#e74c3c' else 'darkblue' for c in colors], 
                linewidth=1)

plt.xlabel('MUSIQ Score (higher is better)', fontsize=12)
plt.ylabel('Model', fontsize=12)
plt.title('MUSIQ Scores by Model', 
...
# 只在条末端显示分数
for i, (bar, mean_val) in enumerate(zip(bars, df2_sorted['MUSIQ_Mean'])):
    width = bar.get_width()
...
x_max = max(df2_sorted['MUSIQ_Mean']) + df2_sorted['MUSIQ_Std'].max() + 0.3
plt.xlim(left=3.0, right=x_max)

plt.tight_layout()
plt.savefig('./data_analysis/MUSIQ_scores_with_exclusion.png', dpi=300, bbox_inches='tight')
# plt.show()