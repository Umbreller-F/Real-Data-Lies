...
from data.hunyuan_vae.vae import DecoderCausal3D, BaseOutput, DecoderOutput, DiagonalGaussianDistribution, EncoderCausal3D
...
class AutoencoderKLCausal3D(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    ...
    def reconstruct(self, frame_paths) -> list[Image.Image]:
        ...
        video_clip.append(video_clip[-1])
        video_tensor = torch.stack(video_clip, dim=0)
        video_tensor = video_tensor.permute(1, 0, 2, 3)
        video_tensor = video_tensor.unsqueeze(0)
        video_tensor = video_tensor.cuda().to(torch.float16)
        z = self.encode(video_tensor).latent_dist.mode()
        recon_video_tensor = self.decode(z).sample.squeeze()
        recon_video_tensor = recon_video_tensor.permute(1, 0, 2, 3)

        recon_imgs = []
        for img in recon_video_tensor:
            img = img * 0.5 + 0.5
